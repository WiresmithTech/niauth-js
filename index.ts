...
import { b64tohex, makeUrlBase64, bigIntToBase64, hexStringToBase64 } from "./lib/Utils";
...
export async function getAggregateUserPermissions(username) {
   let response = await fetch('/LVWSAuthSvc/GetAggregateUserPermissions?username=' + (username || ''), {
...
function decodeServerParamsString(srpParams: string): ServerInfo {
   ...
   const param_strings = splitParamsString(srpParams);
   if (!param_strings.hasOwnProperty('N') ||
       !param_strings.hasOwnProperty('s') ||
       !param_strings.hasOwnProperty('B') ||
       !param_strings.hasOwnProperty('ss')) {
      throw 'didn\'t get everything we needed from server';
   }

   const params: ServerHeaderParams = {
      N: parseInt(param_strings.N, 10),
      s: base64ToByteArray(param_strings.s),
      B: new BigInteger(b64tohex(param_strings.B), 16),
      ss: param_strings.ss
...
async function newLogin(serverParams: string): Promise<boolean> {
   ...
   var clientParams = await srpClient.generatePublicKeyAndProof();

   /* Send the client-side parameters back to the server */
   var data;
   data  = 'A=' + makeUrlBase64(bigIntToBase64(clientParams.clientPublicKey, 128));
